<div class="content home">
    ...
    <aside>
        ...
        <article class="useragent boxaside">
            ...
            $read->FullRead(null, "SELECT SUM(agent_views) AS TotalViews FROM ws_siteviews_agent");
            $TotalViews = $read->getResult()[0]->TotalViews;

            $read->setTable('ws_siteviews_agent');
            $read->Query("ORDER BY agent_views DESC LIMIT 3", null);
            if (!$read->getResult()):
    ...
    <section class="content_statistics">
        ...
        <section>
            ...
            <?php
            $read->setTable("ws_posts");
            $read->Query("ORDER BY post_date DESC LIMIT 3", null);
            if ($read->getResult()):
        ...
        <section>
            ...
            <?php
            $read->setTable("ws_posts");
            $read->Query("ORDER BY post_views DESC LIMIT 3", null);
            if ($read->getResult()):
                foreach ($read->getResult() as $re):
                    extract((array) $re);
                    $status = (!$post_status ? 'style="background: #fffed8"' : '' );
