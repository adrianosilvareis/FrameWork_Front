        ...
        <form name="PostForm" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="label_line">                
                <label class="label_medium">
                    ...
                    <input type="url" placeholder="http://www.site.com.br" name="empresa_site" value="<?php if (isset($data['empresa_site'])) echo $data['empresa_site']; ?>" />
                </label>

                <label class="label_medium">
                    <span class="field">Facebook Page:</span>
                    <input type="text" placeholder="facebook" name="empresa_facebook" value="<?php if (isset($data['empresa_facebook'])) echo $data['empresa_facebook']; ?>" />
                </label>
            </div><!-- line -->

            <label class="label">
                <span class="field">Nome da rua / Número:</span>
                <input type="text" placeholder="Rua Nome da Rua / 123" name="empresa_endereco" value="<?php if (isset($data['empresa_endereco'])) echo $data['empresa_endereco']; ?>" />
            </label>
            ...
            <div class="label_line">
                <label class="label_small">
                    ...
                    <select class="j_loadstate" name="empresa_uf">
                        ...
                        <?php
                        $readState = new Read("app_estados");
                        $readState->Query("ORDER BY estado_nome ASC", null);
                        foreach ($readState->getResult() as $estado):
                            extract((array) $estado);
                            echo "<option value=\"{$estado_id}\" ";
                            if (isset($data['empresa_uf']) && $estado_id == $data['empresa_uf']): echo 'selected'; endif;
                ...
                <label class="label_small">
                    ...
                    <select class="j_loadcity" name="empresa_cidade">
                        <?php if (!isset($data['empresa_cidade'])): ?>
                            ...
                            $City = new Read("app_cidades");
                            $City->Query("WHERE estado_id = :uf ORDER BY cidade_nome ASC", "uf={$data['empresa_uf']}");
                            if ($City->getRowCount()):
